...
smpl_t inputAppend[512];
int appendOffset = 0;
//std::vector<smpl_t> inputAppend;
	      ...
	      double streamTime, RtAudioStreamStatus status, void * userData)
{
    ...
    // Suggested FIX: Accumulate input buffer samples in increments of 128 to new buffer
    // using 128 frame buffer, and 2.9 * 2 output delay
    std::copy(input, input + 127, inputAppend + appendOffset);
    appendOffset += 128;

    if (appendOffset >= 512){
    // Until entire inputAppend buffer of 512 sample frames is copied to buffer 'in'
    while (data->offset < 256)
    {
	std::copy(inputAppend + data->offset, inputAppend + data->offset + 255, in->data);

	// GIST runs FFT automatically to prep input for feature extraction   
	gist.processAudioFrame(inputAppend, 512);
...
main()
{
    ...
    unsigned int aubioBufferFrames = 512;
    unsigned int hop_size     = 256; // Necessary for aubio onset detection
    ...
    o	= new_aubio_onset("complex", aubioBufferFrames, hop_size, sampleRate); 
...
free(dataset);
    ...
    if (out)
	del_fvec(out);
cout << "deleting aubio onset" << endl;
    if (o)
    {
       del_aubio_onset(o);
       o = 0;
